...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/ethereum/go-ethereum/wiki/JavaScript-Console.
This command allows to open a console on a running autonity node.`,
	}
...
// remoteConsole will connect to a remote autonity instance, attaching a JavaScript
// console to it.
...
// for "autonity attach" and "autonity monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with autonity < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
